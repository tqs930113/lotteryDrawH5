$(function() {
	var lottery = {
		...
		stop: function() {
			...
			if (lottery.times > lottery.cycle + 10 && lottery.prize == lottery.index) {
				...
				var objs=new Array();
				var resultContainer = $('#lottery .wrap__content');
				objs.push($('#lottery .layer'));
				objs.push($('#lottery .wrap'));
				controlLayerClose(objs);
				resultContainer.html(`恭喜您！您抽中了第${lottery.place }个奖品`);
	...
	var controlLayerClose=function(objs){
		//控制抽奖后遮罩弹出层的显示与否
		var btn = $(objs) ;
		for(let item of btn){
			...
			$(item).parent().addClass('active').fadeOut(1000)
					.siblings('.layer').addClass('active').fadeOut(1000);
		}

	}
	$('#wrap__close').on('click',function(){
		controlLayerClose(this);
	});
